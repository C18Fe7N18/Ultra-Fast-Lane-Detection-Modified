from utils.loss import SoftmaxFocalLoss, ParsingRelationLoss, ParsingRelationDis, ParsingRelationLines
...
def get_loss_dict(cfg):
    if cfg.use_aux:
        loss_dict = {
            'name': ['cls_loss', 'relation_loss', 'aux_loss', 'relation_dis', 'relation_lines'],
            'op': [SoftmaxFocalLoss(2), ParsingRelationLoss(), torch.nn.CrossEntropyLoss(), ParsingRelationDis(),
                   ParsingRelationLines()],
            'weight': [1.0, cfg.sim_loss_w, 1.0, cfg.shp_loss_w, cfg.shp_lines_w],
            'data_src': [('cls_out', 'cls_label'), ('cls_out',), ('seg_out', 'seg_label'), ('cls_out', 'cls_label'),
                         ('cls_out', 'cls_label')]
        }
    else:
        loss_dict = {
            'name': ['cls_loss', 'relation_loss', 'relation_dis', 'relation_lines'],
            'op': [SoftmaxFocalLoss(2), ParsingRelationLoss(), ParsingRelationDis(), ParsingRelationLines()],
            'weight': [1.0, cfg.sim_loss_w, cfg.shp_loss_w, cfg.shp_lines_w],
            'data_src': [('cls_out', 'cls_label'), ('cls_out',), ('cls_out', 'cls_label'), ('cls_out', 'cls_label')]
        }
